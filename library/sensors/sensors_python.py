...
PNIC_BEFORE = {}
ROCM_SMI_PATH = shutil.which("rocm-smi") or "/opt/rocm/bin/rocm-smi"
...
class GpuAmd(sensors.Gpu):
    @staticmethod
    def _run_rocm_smi(*args) -> str | None:
        if not ROCM_SMI_PATH or not shutil.which(ROCM_SMI_PATH):
            return None
    ...
    @classmethod
    def _stats_from_rocm_smi(cls) -> Tuple[float, float, float, float, float] | None:
        output = cls._run_rocm_smi("--showtemp", "--showuse", "--showmemuse")
        if not output:
            return None
        ...
        match = re.search(r"GPU use \(%\):\s*([0-9]+(?:\.[0-9]+)?)", output)
        if match:
            load = float(match.group(1))

        match = re.search(r"GPU Memory Allocated \(VRAM%\):\s*([0-9]+(?:\.[0-9]+)?)", output)
        if match:
            memory_percentage = float(match.group(1))

        match = re.search(r"Temperature \(Sensor edge\) \(C\):\s*([0-9]+(?:\.[0-9]+)?)", output)
        if match:
            temperature = float(match.group(1))

        return load, memory_percentage, math.nan, math.nan, temperature

    @classmethod
    def _frequency_from_rocm_smi(cls) -> float:
        output = cls._run_rocm_smi("--showclocks")
        if not output:
            return math.nan

        match = re.search(r"sclk clock level:\s*\d+:\s*\(([0-9]+(?:\.[0-9]+)?)Mhz\)", output, re.IGNORECASE)
        if match:
    ...
    def stats() -> Tuple[
        ...
        if pyamdgpuinfo:
            ...
            rocm_stats = GpuAmd._stats_from_rocm_smi()
            if rocm_stats:
                return rocm_stats
    ...
    def frequency() -> float:
        try:
            if pyamdgpuinfo:
                ...
                return GpuAmd._frequency_from_rocm_smi()
        except:
            return math.nan
    ...
    def is_available() -> bool:
        try:
            if pyamdgpuinfo and pyamdgpuinfo.detect_gpus() > 0:
                ...
                return True
            elif GpuAmd._run_rocm_smi("--showtemp"):
                return True
            else:
                return False
        except:
