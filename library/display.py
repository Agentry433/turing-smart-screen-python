...
def _smartmonitor_runtime_enabled() -> bool:
    return config.CONFIG_DATA["display"].get("REVISION") == "A_HID" and \
        config.CONFIG_DATA["display"].get("SMARTMONITOR_HID_RUNTIME", False)
...
class Display:
    def __init__(self):
        ...
        elif config.CONFIG_DATA["display"]["REVISION"] == "A_HID":
            self.lcd = LcdCommRevAHid(com_port=config.CONFIG_DATA['config']['COM_PORT'],
                                      update_queue=config.update_queue)
    ...
    def initialize_display(self):
        if _smartmonitor_runtime_enabled():
            from library import smartmonitor_runtime
            smartmonitor_runtime.initialize_display()
            return
